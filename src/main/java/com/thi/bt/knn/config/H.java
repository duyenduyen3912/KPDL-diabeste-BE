...
public class H {
    ...
    public static Sort getSort(String sortByProperties, String sortBy) {
        ...
        if (Sort.Direction.ASC.toString().equals(sortBy)) {
            sort = Sort.by(sortByProperties.split(",")).ascending();
        } else {
            sort = Sort.by(sortByProperties.split(",")).descending();
        }
        return sort;
    }
...
//        if (value instanceof Number) return !((Number) value).equals(Long.valueOf(0));
    ...
    public static Date convertFromLong(Long fromDateLong) {
        if(fromDateLong == null || fromDateLong == 0 ) return null;
        //check type is timestamp or unixtime
        if (fromDateLong.toString().length() == 10) {
            fromDateLong = fromDateLong * 1000;
        }
